...
class DiamondiaTokenizer:
    def __init__(self, config: DiamondiaConfig):
        ...
        special_tokens = [
            ...
            "<|lang_rust|>", "<|lang_cpp|>", "<|lang_java|>", "<|lang_c|>",
            "<|endofmask|>", "<|retrieved_context|>", "<|endofcontext|>"
        ]
...
class CodeRetriever:
    ...
    def _parse_c_cpp_file(self, file_path: Path) -> List[Tuple[int, int, str]]:
        chunks = []
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        ...
        while i < len(lines):
            line = lines[i].strip()
            if (line.endswith('{') or '{' in line) and any(keyword in line for keyword in [
                'void ', 'int ', 'char ', 'float ', 'double ', 'bool ', 'long ', 'short ', 'unsigned ',
                'static ', 'inline ', 'extern ', 'struct ', 'class ', 'auto '
            ]):
                if not any(w in line for w in ['if(', 'if (', 'for(', 'for (', 'while(', 'while (', 'switch(']):
                    start_line = i
                    brace_count = 0
                    for j in range(i, len(lines)):
                        ...
                        chunk_text = "".join(lines[i:end_line]).strip()
                        chunks.append((i + 1, end_line, chunk_text))
                        i = end_line
            else:
                i += 1
        ...
        if not chunks:
            for i in range(0, len(lines), 10):
                chunk = "".join(lines[i:i+10]).strip()
                if chunk:
                    chunks.append((i+1, min(i+10, len(lines)), chunk))
    ...
    def _load_or_build_index(self):
        ...
        if need_rebuild:
            self.chunks = []
            texts = []
            for ext in ["*.py", "*.js", "*.ts", "*.go", "*.rs", "*.cpp", "*.cxx", "*.c", "*.h", "*.hpp", "*.java"]:
                for file_path in self.codebase_path.rglob(ext):
                    if file_path.suffix in ['.c', '.h', '.cpp', '.cxx', '.hpp']:
                        chunks = self._parse_c_cpp_file(file_path)
                    else:
    ...
    def _compute_codebase_hash(self) -> str:
        ...
        for file_path in sorted(self.codebase_path.rglob("*")):
            if file_path.is_file() and file_path.suffix in {".py", ".js", ".ts", ".go", ".rs", ".cpp", ".cxx", ".c", ".h", ".hpp", ".java"}:
                try:
...
""")
    ...
    def test_c_code(self, code: str, test_cases: List[Tuple[str, Any]] = None) -> Dict[str, Any]:
        if not test_cases:
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.c', delete=False) as f:
            ...
            try:
                ...
                return {
                    "success": success,
                    "passed": 0 if not success else len(test_cases),
                    "total": len(test_cases),
                    "error_type": None if success else "CompileError",
                    "error_message": error_msg,
                }
            except subprocess.TimeoutExpired:
                return {"success": False, "error_type": "Timeout", "error_message": f"Timeout after {self.timeout_sec}s"}
            except FileNotFoundError:
                return {"success": False, "error_type": "NoCompiler", "error_message": "GCC not found"}
            except Exception as e:
                return {"success": False, "error_type": "Runtime", "error_message": str(e)}
            finally:
                try:
...
class CodeDataset(Dataset):
    def __init__(self, codebase_path: str, tokenizer: DiamondiaTokenizer, max_length: int = 1024):
...
class DiamondiaEngine:
    ...
    def generate_and_test_code(
        ...
        is_c_code = "<|lang_c|>" in prompt
        
        if is_c_code:
            test_result = self.code_tester.test_c_code(gen_only, test_cases)
        else:
...
def train_diamondia(
    model: DiamondiaModel,
    tokenizer: DiamondiaTokenizer,
    codebase_path: str,
    epochs: int = 5,
    batch_size: int = 1,
    learning_rate: float = 1e-4,
    max_length: int = 1024,
    save_path: str = "diamondia_finetuned.pt"
):
...
def main():
    ...
    parser.add_argument("--train_data", type=str, default="my_code/", help="Path to training data")
